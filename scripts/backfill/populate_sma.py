#!/usr/bin/env python3
"""
Backfill sma table from Polygon.io API
Fetches historical SMA data from list_date to present
Full reload: TRUNCATE then INSERT all data
Fetches common SMA windows: 20, 50, 200 days
"""
import os
import sys
import time
...
def fetch_sma(ticker, window, from_date=None):
    """Fetch SMA for a ticker and window size"""
...
def populate_table(tickers_data):
    """Fetch and insert SMA for all tickers and windows (TRUNCATE + INSERT)"""
    insert_sql = """
        INSERT INTO sma (
    ...
    with get_psycopg2_connection() as conn:
        with conn.cursor() as cur:
            ...
            logger.info("TRUNCATING sma table...")
            cur.execute("TRUNCATE TABLE sma RESTART IDENTITY CASCADE;")

            # Fetch and insert SMA data
            logger.info(
                f"Fetching SMA data for {len(tickers_data)} tickers with windows {SMA_WINDOWS}..."
            )
            processed = 0
            ...
            for i, (ticker, list_date) in enumerate(tickers_data, 1):
                ...
                for window in SMA_WINDOWS:
                    values = fetch_sma(ticker, window, from_date)
                    ...
                    if values:
                        for value_point in values:
                            ...
                            # Filter by list_date
                            if sma_date and sma_date < from_date:
                                continue

                            sma_value = value_point.get("value")

                            if ticker and sma_date and sma_value is not None:
                                key = (ticker, sma_date, window)
                ...
                if processed % 100 == 0:
                    logger.info(
                        f"  Progress: {processed}/{len(tickers_data)} tickers, {len(unique_records):,} unique SMA values collected"
                    )
            ...
            logger.info(f"Inserting {len(unique_records):,} unique SMA values...")
            batch = []
            batch_size = 1000
            total_inserted = 0
            ...
            logger.info(
                f"Final: {processed} tickers processed, {total_inserted:,} SMA values inserted"
            )
            ...
            stats = cur.fetchone()
            logger.info(f"\nData statistics:")
            logger.info(f"  Total SMA values: {stats[0]:,}")
            logger.info(f"  Unique tickers: {stats[1]}")
            logger.info(f"  Windows: {stats[2]} ({SMA_WINDOWS})")
            ...
            cur.execute(
                ...
                FROM sma
                WHERE date = (SELECT MAX(date) FROM sma)
                ORDER BY ticker, window_size
                LIMIT 15;
            """
            )
            logger.info(f"\nSample SMA values on latest date:")
            for row in cur.fetchall():
                ticker, sma_date, window, val = row
...
def main():
    ...
    try:
        ...
        logger.info("\nBackfill complete: SMA table fully loaded")
