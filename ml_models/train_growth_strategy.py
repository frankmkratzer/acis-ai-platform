#!/usr/bin/env python3
"""
Growth Strategy Model Training
Focus: High price appreciation potential (all market caps with proper filters)
"""
import sys
...
class GrowthStrategyTrainer:
    """
    Growth Strategy Model Trainer

    Target: Stocks with high price appreciation potential
    Universe: Small-cap+ (>$300M), Price >$5 for small-caps, >$0.50 for mid/large
    """

    def __init__(self, gpu_id: int = None, market_cap_segment: str = "all"):
        self.gpu_id = gpu_id
        self.strategy = "growth"
        self.market_cap_segment = market_cap_segment  # "small", "mid", "large", "all"
    ...
    def load_features(self, start_date: date, end_date: date) -> pd.DataFrame:
        """Load features with growth strategy filters"""
        logger.info(
            f"Loading growth strategy features ({self.market_cap_segment} cap): {start_date} to {end_date}"
        )
        logger.info(
        ...
        # NOTE: Market cap filter removed for training due to sparse fundamental data
        # Market cap filtering will be applied at inference time in portfolio generation
        logger.warning(
            "⚠️  Training on ALL stocks regardless of market cap (fundamental data is sparse)"
        )
        logger.warning("⚠️  Market cap filtering will be applied during portfolio generation")
    ...
    def prepare_features(self, df: pd.DataFrame):
        """Prepare features with growth focus"""
        # Key growth features (higher importance)
        growth_features = [
            "return_1d",
            "return_5d",
            "return_20d",  # Momentum
            ...
            "macd_line",
            "macd_signal",
            "macd_histogram",  # Technical momentum
            "volume_ratio_20d",  # Volume breakout
            "roe",
            "roa",  # Profitability
            "pe_ratio",
            "pb_ratio",  # Valuation (lower better for growth)
        ]
    ...
    def train(self, X, y, feature_names):
        """Train XGBoost with growth strategy parameters"""
        logger.info("Training growth strategy model...")
    ...
    def save_model(self, model, feature_names, metrics):
        """Save trained model"""
        model_name = f"growth_{self.market_cap_segment}cap"
...
def main():
    parser = argparse.ArgumentParser(description="Train growth strategy model")
    ...
    logger.info("=" * 80)
    logger.info("GROWTH STRATEGY MODEL TRAINING")
    logger.info(f"Market Cap Segment: {args.market_cap.upper()}")
    logger.info("Focus: High price appreciation potential")
    logger.info("=" * 80)
    ...
    trainer = GrowthStrategyTrainer(gpu_id=args.gpu, market_cap_segment=args.market_cap)
    ...
    importance.to_csv(
        f"ml_models/feature_importance/feature_importance_growth_{args.market_cap}cap.csv",
        index=False,
    )

    logger.info("=" * 80)
    logger.info(f"✅ GROWTH STRATEGY MODEL TRAINING COMPLETE ({args.market_cap.upper()} CAP)!")
    logger.info("=" * 80)
    logger.info(f"Spearman IC: {metrics['spearman_ic']:.4f}")
    logger.info(f"Model saved to: models/growth_{args.market_cap}cap/")
    logger.info("=" * 80)
